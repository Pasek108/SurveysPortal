        ...
        <div class="relative flex flex-col items-start max-w-screen-xl mx-auto md:flex-row">
            ...
            <div class="flex flex-col w-full p-5 md:w-2/3 lg:w-3/4 min-h">
                ...
                <div class="mt-10">
                                ...
                                <tr class="text-left text-white divide-x bg-slate-800">
                                    ...
                                    <th class="w-1 px-4 py-2 whitespace-nowrap">
                                        ...
                                        @if ((empty(request()->sort) || request()->sort == 'user') && request()->order == 'DESC')
                                            <a href="{{ url()->current() }}?sort=user&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-up-wide-short"></i>
                                            </a>
                                        @elseif ((empty(request()->sort) || request()->sort == 'user') && request()->order == 'ASC')
                                            <a href="{{ url()->current() }}?sort=user&order=DESC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-down-short-wide"></i>
                                            </a>
                                        @else
                                            <a href="{{ url()->current() }}?sort=user&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                    ...
                                    <th class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        @if ((empty(request()->sort) || request()->sort == 'reason') && request()->order == 'DESC')
                                            <a href="{{ url()->current() }}?sort=reason&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-up-wide-short"></i>
                                            </a>
                                        @elseif ((empty(request()->sort) || request()->sort == 'reason') && request()->order == 'ASC')
                                            <a href="{{ url()->current() }}?sort=reason&order=DESC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-down-short-wide"></i>
                                            </a>
                                        @else
                                            <a href="{{ url()->current() }}?sort=reason&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                    ...
                                    <th class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        @if ((empty(request()->sort) || request()->sort == 'end_date') && request()->order == 'DESC')
                                            <a href="{{ url()->current() }}?sort=end_date&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-up-wide-short"></i>
                                            </a>
                                        @elseif ((empty(request()->sort) || request()->sort == 'end_date') && request()->order == 'ASC')
                                            <a href="{{ url()->current() }}?sort=end_date&order=DESC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-down-short-wide"></i>
                                            </a>
                                        @else
                                            <a href="{{ url()->current() }}?sort=end_date&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                    ...
                                    <th class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        @if ((empty(request()->sort) || request()->sort == 'created_at') && request()->order == 'DESC')
                                            <a href="{{ url()->current() }}?sort=created_at&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-up-wide-short"></i>
                                            </a>
                                        @elseif ((empty(request()->sort) || request()->sort == 'created_at') && request()->order == 'ASC')
                                            <a href="{{ url()->current() }}?sort=created_at&order=DESC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                                <i class="ml-1 text-sm cursor-pointer fa-solid fa-arrow-down-short-wide"></i>
                                            </a>
                                        @else
                                            <a href="{{ url()->current() }}?sort=created_at&order=ASC&search={{ request()->search }}&user_id={{ request()->user_id }}">
                                ...
                                @foreach ($bans as $ban)
                                    <tr class="text-left bg-zinc-100 even:bg-gray-300 ">
                                        ...
                                        <td class="px-4 py-2 whitespace-nowrap">
                                            <a href="{{ url()->current() }}?sort={{ request()->sort }}&order={{ request()->order }}&search={{ request()->search }}&bans_page={{ $bans->currentPage() }}" class="px-1.5 py-0.5 font-bold text-white bg-blue-700 rounded-md hover:bg-blue-800 whitespace-nowrap">
                    ...
                    <div class="w-full mt-3">{{ $bans->appends(['sort' => request()->sort, 'order' => request()->order, 'search' => request()->search])->onEachSide(1)->links() }}</div>
                </div>
            </div>
        </div>
    </section>
